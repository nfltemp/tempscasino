...
-- Function to create a new deck (MOVED TO TOP)
local function createDeck()
    local deck = {}
    -- Using card-specific emojis
    local suits = {
        hearts = {
            symbol = "♥️",
            cards = {
    ...
    -- Create deck with card emojis and their values
    for suitName, suitData in pairs(suits) do
        for _, value in ipairs(values) do
            table.insert(deck, {
                value = value,
                suit = suitData.symbol,
                display = suitData.cards[value] -- Use the specific card emoji
            })
        end
    end
...
-- Function to calculate hand value (MOVED UP)
...
-- Function to format hand for display (MOVED UP)
...
-- Then define the functions in order
...
-- Finally, the CreateThread for the ped
CreateThread(function()
    ...
    -- Add blip for the casino
    local blip = AddBlipForCoord(1993.972, 3050.766, 46.215)
    SetBlipSprite(blip, 679)  -- Casino/cards sprite
    SetBlipDisplay(blip, 4)
    SetBlipScale(blip, 0.8)
    SetBlipColour(blip, 50)  -- Purple color
